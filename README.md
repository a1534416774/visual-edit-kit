...
早期版本有一个很迷惑的 BUG：**调完大小/位置之后，点页面任何地方都再也叫不出「页面微调」面板**。
根因有三个，现已全部修掉，并加了 headless 真机回归测试（`packages/ve-core/test_panel_visibility.py`，28 项断言）：

1. **面板被关掉后永远打不开**（主因）。`deselect()` 会把面板置 `display:none`，而再次选中元素时
   `buildPanel()` 只是把旧节点 `appendChild` 回去、没有恢复 `display`。
   修复：新增 `showPanel()`，`select()` 时显式恢复显示。
2. **绿框吃掉了所有点击**。选中元素后，覆盖在它上面的绿色可拖拽层（`pointer-events:auto`）
   会拦住其内部所有子元素（甚至整页，如果你把它拉大了）的点击，于是「点哪儿都没反应、面板也不出现」。
   修复：绿框上的**单击（非拖拽）做穿透点选**——用 `elementsFromPoint` 跳过自家 UI，命中最内层元素并选中它
   （想选父级用「🗂 图层」）。
3. **开关按钮的语义陷阱**。面板不可见时点右下角开关会**把整个微调模式关掉**，看起来更像"功能没了"。
   修复：开着但面板不可见时，点开关 = **把面板找回来**（有上次选中就恢复，没有就提示"点击页面元素打开面板"）；
   只有在面板可见时点开关才是关模式。

顺带增强：

- **面板可拖动**：拖标题栏任意摆放（位置记在 `localStorage`，双击标题栏恢复"自动跟随选中元素"）；
  自动摆位优先级 = 元素下方 → 上方 → 右侧 → 左侧 → 贴右上，并且**永远完整落在视口内**
  （面板改为 `position:fixed` + `max-height:100vh-16px` + 内部滚动，宿主页面 `body{overflow:hidden}` 也不会被裁掉）。
- **面板跟随**：宿主窗口/内部容器滚动、窗口缩放时，绿框与面板都会跟着选中元素重新定位。
- **开关按钮 z-index 提到面板之上**，避免被面板压住而"入口消失"。
- 新增只读 API 便于自动化测试与外部集成：`getActive() / select(el) / panelVisible() / isOn() / resetPanelPos()`。
